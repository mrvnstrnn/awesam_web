                        ...
                        <div class="main-card mb-3 card">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    @php
                                        $activity_id = \Auth::user()->getUserProgramAct(4, $program->program_id);
                                    @endphp
                                    <table id="new-endoresement-{{ strtolower(str_replace(" ", "-", $program->program))  }}-table" class="align-middle mb-0 table table-borderless table-striped table-hover new-endorsement-table" data-href="{{ route('all.getDataNewEndorsement', [\Auth::user()->profile_id, $program->program_id, 4, $activity_id->what_to_load]) }}">
                                        <thead>
                                            <tr>
                                                <th style="width: 15px;">
                            ...
                            <div class="d-block text-right card-footer">
                                <button type="button" class="btn btn btn-outline-danger btn-bulk-acceptreject-endorsement" data-program="{{ strtolower($program->program) }}" data-id="{{ $program->program_id }}" data-complete="false" id="reject{{ strtolower(str_replace(" ", "-", $program->program))  }}" data-href="{{ route('accept-reject.endorsement') }}" data-activity_name="endorse_site">Reject</button>
                                <button type="button" class="btn btn-primary btn-bulk-acceptreject-endorsement" data-program="{{ strtolower($program->program) }}" data-id="{{ $program->program_id }}" data-complete="true" id="accept{{ strtolower(str_replace(" ", "-", $program->program))  }}" data-href="{{ route('accept-reject.endorsement') }}" data-activity_name="endorse_site">Endorse New Sites</button>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('js_script')
    ...
    <script src="{{ asset('js/program-staff.js') }}"></script>
@endsection
