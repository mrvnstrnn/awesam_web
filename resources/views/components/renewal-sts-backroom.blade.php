...
<script>
    $(document).ready(function(){
        $.ajax({
            url: "/get-form/" + "826" + "/" + "{{ $site[0]->activity_name }}",
            method: "GET",
            // headers: {
    ...
    $(".form_html").on("click", ".save_endorsement_to_sts_backroom_btn", function(e) {
        e.preventDefault();
        ...
        $.ajax({
            url: "/save-endorsment-to-sts",
            method: "POST",
            data: $(".endorsement_to_sts_backroom_form, .site_data_form").serialize(),
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function (resp) {
                if (!resp.error){
                    ...
                    $("table[data-program_id="+"{{ $site[0]->program_id }}"+"]").DataTable().ajax.reload(function(){
                        ...
                        $(".save_endorsement_to_sts_backroom_btn").removeAttr("disabled");
                        $(".save_endorsement_to_sts_backroom_btn").text("Save");
